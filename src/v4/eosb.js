...
function seller() {
  try {
    ...
    if (s.company?.nameEn) {
      return { nameEn: s.company.nameEn, nameAr: s.company.nameAr || '', cr: s.company.cr || '' };
    }
  } catch (_e) {
    /* fallback */
  }
  return { nameEn: SEED_COMPANY.nameEn, nameAr: SEED_COMPANY.nameAr, cr: SEED_COMPANY.crNo };
}
  ...
  return `<div class="inv-doc">
    <div class="inv-head">
      <div><div class="inv-title">${currentLang() === 'ar' ? s.nameAr || s.nameEn : s.nameEn}</div>
        <div style="font-size:12px;color:var(--text-muted)" dir="ltr">CR ${s.cr || '—'}</div></div>
